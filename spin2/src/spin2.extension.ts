...
class Spin2DocumentSemanticTokensProvider implements vscode.DocumentSemanticTokensProvider {
                    ...
                    for (let index = 0; index < possNames.length; index++) {
                        ...
                        if (possibleName.substr(0, 1).match(/[a-zA-Z_]/)) {
                            ...
                            if (referenceDetails != undefined) {
                                ...
                                if (!this._isSpinReservedWord(namePart) && !this._isBuiltinReservedWord(namePart) && !this._isDebugMethod(namePart) && !this._isDebugSymbol(namePart) && !this._isUnaryOperator(namePart)) {
    ...
    private spinDebugLogEnabled: boolean = false; //true;    // WARNING (REMOVE BEFORE FLIGHT) disable before commit
