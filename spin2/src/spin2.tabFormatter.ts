    ...
    */
    ...
    indentTabStop(document: vscode.TextDocument, selections: readonly vscode.Selection[]): vscode.ProviderResult<vscode.TextEdit[]> {
        return selections.map(selection => {
            ...
            this._logMessage(`inD selection(isSingle-[${selection.isSingleLine}] BLOK-[${block}] isEmpty-[${selection.isEmpty}] s,e-[${selection.start.line}:${selection.start.character} - ${selection.end.line}:${selection.end.character}] activ-[${selection.active.character}] anchor-[${selection.anchor.character}])`);
            ...
            if (selectedText.length > 0) {
                ...
                this._logMessage(`inD  bRplcLeft=[${bReplaceLeftOfSelection}] ltEdge-[${cursorPos.line}:${cursorPos.character}] text-[${selectedText}] rplcCt=(${replaceCount})`);
                ...
                if (bReplaceLeftOfSelection) {
                    nextTabstop = this.getNextTabStop(block, selection.active.character);
                }
                this._logMessage(`inD finding: BLOK-[${block}] next TabStop is TAB-[${nextTabstop}]`);
                ...
                if (nextTabstop > cursorPos.character) {
                    ...
                    return [
                        vscode.TextEdit.insert(cursorPos, charactersToInsert)
                    ];
                } else {
    ...
    outdentTabStop(document: vscode.TextDocument, selections: readonly vscode.Selection[]): vscode.ProviderResult<vscode.TextEdit[]> {
        return selections.map(selection => {
            const block = this.getBlockName(document, selection);
            this._logMessage(`outD selection(isSingle-[${selection.isSingleLine}] BLOK-[${block}] isEmpty-[${selection.isEmpty}] s,e-[${selection.start.line}:${selection.start.character} - ${selection.end.line}:${selection.end.character}] activ-[${selection.active.character}] anchor-[${selection.anchor.character}])`);
